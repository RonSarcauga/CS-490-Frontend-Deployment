    ...
    async bookAppointment(dateInput, timeInput, doctorId, patientId) {
        try {
            ...
            if (hours < 9 || hours > 21 || (hours === 21 && minutes > 0)) {
                alert("Invalid appointment time: Appointments can only be scheduled between 09:00 and 21:00.");
                return;
            }
            ...
            if (isDateConflict) {
                alert("Appointment conflict: You already have an appointment scheduled on this date.");
                return;
            }
